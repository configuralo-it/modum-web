...
function ProductObject({ reduced }: { reduced: boolean }) {
  ...
  useFrame((state) => {
    ...
    group.current.rotation.x = -0.12 + state.pointer.y * 0.08;
    group.current.rotation.y = 0.35 + state.pointer.x * 0.1;
    group.current.rotation.z = 0.18 + state.pointer.x * 0.035;
    group.current.position.y = state.pointer.y * 0.025;
  });
  ...
  return (
    <group ref={group} rotation={[-0.12, 0.35, 0.18]} scale={0.86}>
      ...
      <mesh position={[0.35, -0.05, 0.43]} rotation={[Math.PI / 2, 0.16, 0.2]}>
        <torusGeometry args={[0.72, 0.045, 10, 52, Math.PI * 1.42]} />
        <meshStandardMaterial
          color="#ff4d00"
          emissive="#ff4d00"
  ...
  return (
    <div className="hero-canvas" aria-hidden="true">
      ...
      >
        ...
        <directionalLight position={[-4, 5, 6]} intensity={2.55} color="#fff8ee" />
        <directionalLight position={[5, -1, 4]} intensity={1.32} color="#dfe7ff" />
        <pointLight position={[1.8, -1.3, 3]} intensity={3.2} color="#ff4d00" distance={5.5} />
